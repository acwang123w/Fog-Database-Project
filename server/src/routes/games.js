...
router.get('/', async (req,res) => {
  ...
  sql += ` ORDER BY g.title ASC LIMIT 100`;
...
router.get('/:id', async (req,res) => {
  const { rows } = await query(
    `SELECT g.*, d.name AS developer_name
     FROM steam.games g LEFT JOIN steam.developers d USING(developer_id)
     WHERE g.game_id=$1`, [req.params.id]);
...
router.get('/:id/reviews', async (req,res) => {
  const { rows } = await query(
    `SELECT r.*, u.email AS user_email
     FROM steam.reviews r JOIN steam.users u USING(user_id)
     WHERE r.game_id=$1 ORDER BY r.date DESC`, [req.params.id]);
  res.json(rows);
});